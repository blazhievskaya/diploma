...
csvdata$m = csvdata$m1+csvdata$m2+csvdata$m3
genderfields = c("m", "f", "n")
...
allfields = list(genderfields, numberfields, formfields, syntaxfields)
...
for (fields in allfields){
  ...
  while (count<40) {
    ...
    print(
      ggplot(data.m, aes(verb, value+1, fill=variable)) +
      scale_y_log10() +
      geom_bar(position = "dodge", stat="identity")
    )
...
data=csvdata[c("verb", percentfields)];
count = 1;
len = 10;
while (count<40) {
  start = count
